...
var startingStateHandlers = {
    ...
    'getTrucksListOfSameCuisineIntent': function() {
        ...
        if (targetFoodTruckList.length<3){
            var outputResult = targetFoodTruckList.toString();
            this.attributes['numberOfFoodTrucksHasGiven'] = targetFoodTruckList.length;
            var repromt_getTruckList = "You can pick a food truck or get other cuisine food truck lists."
            var output = "There is only " + targetFoodTruckList.length.toString() + " food trucks gave us the valid address information. There are: " + outputResult;
        }
        else{
            ...
            var repromt_getTruckList = "You can get more food truck list by saying:'Give me more food trucks info', or find out a food truck location by saying:'Where is your chonse food truck?'";
        }
    ...
    'getTruckLocationIntent': function() {
        ...
        getWebsiteInString(foodTruckName, function(htmlPage) {
          ...
          if (locationInfo) {
            ...
            this.attributes['foodTruckName'] = null;
            this.attributes['forLocationRepeat'] = null;
            this.handler.state = states.TRUCKLOCATIONMODE;
...
var truckListHandlers = Alexa.CreateStateHandler(states.TRUCKLISTMODE, {
    ...
    'getMoreFoodTrucksIntent': function() {
        ...
        if (start >= targetFoodTruckList.length) {
            var output = "That's all the food trucks with valid location info. Do you want to pick a food truck?";
            var repromt_output = "You can got the food truck location by saying:' where is your chosen food truck?'";
            this.emit(":ask", output, repromt_output);
